...
namespace NvgSharp.Samples
{
	public class Demo
	{
		...
		public void drawWindow(NvgContext vg, string title, float x, float y, float w, float h)
		{
			...
			// Drop shadow
			shadowPaint = vg.BoxGradient(x, y + 2, w, h, cornerRadius * 2, 10, Utility.FromRGBA(0, 0, 0, 128), Utility.FromRGBA(0, 0, 0, 0));
			vg.BeginPath();
		...
		public void drawSearchBox(NvgContext vg, string text, float x, float y, float w, float h)
		{
			...
			// Edit
			bg = vg.BoxGradient(x, y + 1.5f, w, h, h / 2, 5, Utility.FromRGBA(0, 0, 0, 16), Utility.FromRGBA(0, 0, 0, 92));
			vg.BeginPath();
			vg.RoundedRect(x, y, w, h, cornerRadius);
			vg.FillPaint(bg);
			vg.Fill();
		...
		public static void drawEditBoxBase(NvgContext vg, float x, float y, float w, float h)
		{
			Paint bg;
			// Edit
			bg = vg.BoxGradient(x + 1, y + 1 + 1.5f, w - 2, h - 2, 3, 4, Utility.FromRGBA(255, 255, 255, 32), Utility.FromRGBA(32, 32, 32, 32));
			vg.BeginPath();
		...
		public void drawButton(NvgContext vg, string preicon, string text, float x, float y, float w, float h, Color col)
		{
			...
			bg = vg.LinearGradient(x, y, x, y + h, Utility.FromRGBA(255, 255, 255, isBlack(col) ? (byte)16 : (byte)32), 
		...
		public static void drawSlider(NvgContext vg, float pos, float x, float y, float w, float h)
		{
			...
			// Slot
			bg = vg.BoxGradient(x, cy - 2 + 1, w, 4, 2, 2, Utility.FromRGBA(0, 0, 0, 32), Utility.FromRGBA(0, 0, 0, 128));
			vg.BeginPath();
			...
			// Knob Shadow
			bg = vg.RadialGradient(x + (int)(pos * w), cy + 1, kr - 3, kr + 3, Utility.FromRGBA(0, 0, 0, 64), Utility.FromRGBA(0, 0, 0, 0));
			vg.BeginPath();
			...
			// Knob
			knob = vg.LinearGradient(x, cy - kr, x, cy + kr, Utility.FromRGBA(255, 255, 255, 16), Utility.FromRGBA(0, 0, 0, 16));
			vg.BeginPath();
		...
		public static void drawEyes(NvgContext vg, float x, float y, float w, float h, float mx, float my, float t)
		{
			...
			bg = vg.LinearGradient(x, y + h * 0.5f, x + w * 0.1f, y + h, Utility.FromRGBA(0, 0, 0, 32), Utility.FromRGBA(0, 0, 0, 16));
			vg.BeginPath();
			...
			bg = vg.LinearGradient(x, y + h * 0.25f, x + w * 0.1f, y + h, Utility.FromRGBA(220, 220, 220, 255), Utility.FromRGBA(128, 128, 128, 255));
			vg.BeginPath();
			...
			gloss = vg.RadialGradient(lx - ex * 0.25f, ly - ey * 0.5f, ex * 0.1f, ex * 0.75f, Utility.FromRGBA(255, 255, 255, 128), Utility.FromRGBA(255, 255, 255, 0));
			vg.BeginPath();
			vg.Ellipse(lx, ly, ex, ey);
			vg.FillPaint(gloss);
			vg.Fill();

			gloss = vg.RadialGradient(rx - ex * 0.25f, ry - ey * 0.5f, ex * 0.1f, ex * 0.75f, Utility.FromRGBA(255, 255, 255, 128), Utility.FromRGBA(255, 255, 255, 0));
			vg.BeginPath();
			vg.Ellipse(rx, ry, ex, ey);
			vg.FillPaint(gloss);
			vg.Fill();
		...
		public static void drawGraph(NvgContext vg, float x, float y, float w, float h, float t)
		{
			...
			for (i = 0; i < 6; i++)
			{
				bg = vg.RadialGradient(sx[i], sy[i] + 2, 3.0f, 8.0f, Utility.FromRGBA(0, 0, 0, 32), Utility.FromRGBA(0, 0, 0, 0));
				vg.BeginPath();
		...
		public static void drawThumbnails(NvgContext vg, float x, float y, float w, float h, Texture2D[] images, float t)
		{
			...
			// Drop shadow
			shadowPaint = vg.BoxGradient(x, y + 4, w, h, cornerRadius * 2, 20, Utility.FromRGBA(0, 0, 0, 128), Utility.FromRGBA(0, 0, 0, 0));
			vg.BeginPath();
			vg.Rect(x - 10, y - 10, w + 20, h + 30);
			vg.RoundedRect(x, y, w, h, cornerRadius);
			vg.PathWinding(Solidity.Hole);
			...
			for (i = 0; i < images.Length; i++)
			{
				...
				shadowPaint = vg.BoxGradient(tx - 1, ty, thumb + 2, thumb + 2, 5, 3, Utility.FromRGBA(0, 0, 0, 128), Utility.FromRGBA(0, 0, 0, 0));
				vg.BeginPath();
			...
			fadePaint = vg.LinearGradient(x, y + h, x, y + h - 6, Utility.FromRGBA(200, 200, 200, 255), Utility.FromRGBA(200, 200, 200, 0));
			vg.BeginPath();
			...
			// Scroll bar
			shadowPaint = vg.BoxGradient(x + w - 12 + 1, y + 4 + 1, 8, h - 8, 3, 4, Utility.FromRGBA(0, 0, 0, 32), Utility.FromRGBA(0, 0, 0, 92));
			vg.BeginPath();
			...
			shadowPaint = vg.BoxGradient(x + w - 12 - 1, y + 4 + (h - 8 - scrollh) * u - 1, 8, scrollh, 3, 4, Utility.FromRGBA(220, 220, 220, 255), Utility.FromRGBA(128, 128, 128, 255));
			vg.BeginPath();
		...
		public static void drawColorwheel(NvgContext vg, float x, float y, float w, float h, float t)
		{
			...
			paint = vg.BoxGradient(r0 - 3, -5, r1 - r0 + 6, 10, 2, 4, Utility.FromRGBA(0, 0, 0, 128), Utility.FromRGBA(0, 0, 0, 0));
			vg.BeginPath();
